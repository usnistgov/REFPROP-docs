...
As an example, consider Cp, which could be calculated from any equation of state using the quality, but thermodynamically is not defined for a two-phase mixture. Cp is the heat capacity along an isobaric process and is equal to dH/dT|p. Since pressure and temperature do not change across the two phase for a pure fluid, then that means Cp would be equal to infinity because the heat capacity changes but the temperature does not, thus the definition makes it thermodynamically impossible to calculate it.&nbsp; The only place that Cp can be infinity is at the critical point.
